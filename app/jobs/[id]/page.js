      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 px-4">
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="h-12 w-12 md:h-16 md:w-16 border-4 border-indigo-200 rounded-full"></div>
            <div className="absolute top-0 left-0 h-12 w-12 md:h-16 md:w-16 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 text-gray-900 overflow-hidden relative">
      ...
      <div className="sticky top-0 bg-white/90 backdrop-blur-2xl border-b border-gray-200/50 z-50 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 md:py-4 flex items-center justify-between">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-indigo-700 hover:text-indigo-900 transition-colors duration-300 font-medium text-sm"
          >
            <ArrowLeft size={18} />
            <span className="hidden xs:inline">Back to Listings</span>
            <span className="xs:hidden">Back</span>
      ...
      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-8 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 relative z-10">
        {/* Left Section */}
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          {/* Job Header */}
          <div className="relative overflow-hidden rounded-xl md:rounded-2xl bg-white shadow-lg p-5 md:p-8 hover:shadow-xl transition-shadow duration-500 border border-gray-100">
            <div className="flex flex-col sm:flex-row items-start justify-between mb-5 md:mb-6 gap-4">
              <div className="flex items-start gap-3 md:gap-6">
                <div className="relative w-14 h-14 md:w-20 md:h-20 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg overflow-hidden">
                  <Building2 className="text-white w-6 h-6 md:w-8 md:h-8" />
                  <Sparkles className="absolute -top-1 -right-1 text-yellow-400 w-4 h-4 md:w-5 md:h-5" />
                </div>

                <div className="flex-1">
                  <h1 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">{job.title}</h1>
                  <div className="flex items-center gap-2 md:gap-3 mt-1 md:mt-2">
                    <p className="text-base md:text-lg text-gray-700 font-semibold">{job.company}</p>
                  </div>

                  <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-2 md:mt-3 text-xs text-gray-600">
              ...
              {job.featured && (
                <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-100 to-orange-100 border border-amber-400 text-amber-800 font-semibold text-sm">
                  <Zap size={14} />
                  Featured
                </span>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mt-6 md:mt-8">
              <InfoItem icon={<span className="text-base font-bold">₨</span>} label="Salary Range" value={job.salary || "Competitive"} />
              <InfoItem icon={<Briefcase size={18} />} label="Type" value={job.type} />
              <InfoItem icon={<Layers size={18} />} label="Experience" value={`${job.experience} years`} />
              <InfoItem icon={<Calendar size={18} />} label="Start Date" value={new Date(job.startDate).toLocaleDateString()} />
              <InfoItem icon={<AlertCircle size={18} />} label="Deadline" value={new Date(job.endDate).toLocaleDateString()} />
            </div>
          </div>

          {/* Description */}
          <div className="rounded-xl md:rounded-2xl bg-white shadow-lg p-5 md:p-8 border border-gray-100">
            <h2 className="text-lg md:text-xl font-bold mb-4 md:mb-6 text-gray-900">
              About This Role
            </h2>
            ...
            {job.skills?.length ? (
              <div className="flex flex-wrap gap-2 md:gap-3">
                {job.skills.map((skill, i) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 md:px-4 md:py-2 rounded-lg bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 text-indigo-800 font-medium text-sm flex items-center gap-1"
                  >
        ...
        {/* Right Sidebar */}
        <div className="space-y-6 md:space-y-8">
          <div className="sticky top-20 md:top-24 rounded-xl md:rounded-2xl bg-white shadow-lg p-5 md:p-6 border border-gray-100">
            ...
            <button
              disabled={isClosed || loading}
              onClick={handleApplyClick}
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 disabled:from-gray-400 disabled:to-gray-500 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 disabled:hover:scale-100"
            >
              {isClosed ? "Closed" : "Apply Now"}
            </button>

            <div className="mt-4 md:mt-6 space-y-2 text-center text-gray-600 text-sm">
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 flex justify-center gap-4">
              <button onClick={handleShare} className="flex flex-col items-center gap-1 text-gray-600 hover:text-indigo-600 transition">
                <Share2 size={18} />
                <span className="text-xs">Share</span>
              </button>
            </div>
      ...
      {/* Floating Mobile Apply Button */}
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-white/95 backdrop-blur-lg border-t border-gray-200 p-3 shadow-xl z-40">
      ...
      {showProfileReview && profileData && (
        ...
        >
          <div
            className="bg-white/98 backdrop-blur-2xl rounded-xl md:rounded-2xl shadow-2xl max-w-4xl max-h-[90vh] overflow-y-auto w-full border border-white/50"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 md:p-8">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 md:mb-6 gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-r from-indigo-500 to-sky-500 rounded-lg flex items-center justify-center shadow-md">
              ...
              <div className="grid lg:grid-cols-2 gap-5 md:gap-6 mb-6">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="space-y-3 p-3 bg-gradient-to-r from-indigo-50 to-sky-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-900 truncate">{profileData.name || "Not provided"}</p>
                          <p className="text-gray-600 text-sm truncate">{profileData.email}</p>
                          {profileData.phone && <p className="text-gray-600 text-sm truncate">{profileData.phone}</p>}
                        </div>
                      </div>
                      {profileData.about ? (
                        <p className="text-gray-700 italic text-sm">"{profileData.about}"</p>
                      ) : (
                  ...
                  {profileData.resume ? (
                    <div>
                      ...
                      <div className="p-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg border-2 border-dashed border-orange-300">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-14 bg-gradient-to-br from-red-500 to-orange-600 rounded flex items-center justify-center text-white font-semibold text-sm shadow-md">
                              PDF
                            </div>
                            <div className="min-w-0">
                              <p className="font-semibold text-gray-900 truncate text-sm">{profileData.resume.originalName}</p>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-3 py-1.5 bg-orange-600 text-white rounded hover:bg-orange-700 transition text-sm font-medium whitespace-nowrap"
                          >
                            View
                          </a>
                        </div>
                    ...
                    <div className="p-3 bg-red-50 border-2 border-dashed border-red-300 rounded-lg">
                      ...
                      <div className="p-3 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg">
                        <div className="flex flex-wrap gap-2">
                          {profileData.skills.slice(0, 12).map((skill, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-1.5 bg-white border border-emerald-300 text-emerald-800 font-medium rounded text-sm"
                            >
                              {skill.name}
                          ...
                          {profileData.skills.length > 12 && (
                            <span className="px-2 py-1.5 bg-emerald-100 text-emerald-700 font-semibold rounded text-sm">
                              +{profileData.skills.length - 12} more
                            </span>
                          )}
                        </div>
                        {job.skills && (
                          <div className="mt-3 pt-3 border-t border-emerald-200">
                            <p className="text-sm font-medium text-emerald-800 mb-1">Skill Match with Job</p>
                            <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="space-y-2">
                        {profileData.education?.length > 0 ? (
                          profileData.education.slice(0, 2).map((edu, idx) => (
                            <div key={idx} className="p-2 bg-gradient-to-r from-blue-50 to-cyan-50 rounded">
                              <p className="font-medium text-gray-800 text-sm">{edu.degree}</p>
                              <p className="text-xs text-gray-600">{edu.institute}</p>
                            </div>
                          ))
                        ) : (
                          <p className="text-gray-500 text-sm">No education</p>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="space-y-2">
                        {profileData.experience?.length > 0 ? (
                          profileData.experience.slice(0, 2).map((exp, idx) => (
                            <div key={idx} className="p-2 bg-gradient-to-r from-purple-50 to-pink-50 rounded">
                              <p className="font-medium text-gray-800 text-sm">{exp.title}</p>
                              <p className="text-xs text-gray-600">{exp.company}</p>
                            </div>
                          ))
                        ) : (
                          <p className="text-gray-500 text-sm">No experience</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                <Link
                  href="/profile"
                  className="flex-1 bg-gradient-to-r from-gray-600 to-gray-800 hover:from-gray-700 hover:to-gray-900 text-white px-4 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 text-sm"
                >
                ...
                <button
                  onClick={handleConfirmApply}
                  disabled={loading}
                  className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:from-gray-400 disabled:to-gray-500 text-white px-4 py-3 rounded-lg font-semibold shadow-lg hover:shadow-xl disabled:shadow-none transition-all flex items-center justify-center gap-2 text-sm"
                >
                  {loading ? (
                    <>
...
function InfoItem({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3 bg-gradient-to-br from-gray-50 to-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow transition">
      <div className="text-indigo-600 bg-indigo-100 p-2 rounded">
        {icon}
      </div>
      <div>
