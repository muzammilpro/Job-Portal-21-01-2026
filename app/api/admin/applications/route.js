...
export async function GET() {
  try {
    ...
    const jobs = await Job.find({ "applications.0": { $exists: true } })
      .populate("applications.userId", "name email image resume firstName lastName")
      .sort({ createdAt: -1 });
    ...
    console.error("Admin applications GET error: - route.js:52", error);
...
export async function PATCH(req) {
  try {
    ...
    console.error("Admin applications PATCH error: - route.js:98", error);
    return NextResponse.json(
